...
const BucketBottom = ({ activeBucket, didUpdate, setDidUpdate }) => {
    ...
    const [downArrowImage, setDownArrowImage] = useState(images.DownArrow6);
    const [upArrowImage, setUpArrowImage] = useState(images.UpArrow6);
    ...
    const setBucket = () => {
        ...
        if (items.length > itemsPerPage && items.length % itemsPerPage !== 0) {
            if (currentPage === totalPages) {
                setDownArrowImage(images.DownArrow6);
            } else {
    ...
    const handleUpArrowClick = () => {
        if (currentPage > 1) {
            setCurrentPage(currentPage - 1);
            setUpArrowImage(images.UpArrow6);
            if (currentPage - 1 === 1) {
                setUpArrowImage(images.UpArrow6);
            }
        } else {
