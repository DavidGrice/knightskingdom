"""Diagnostic: dump FACETS chunk raw words to inspect incomplete parsing.

Reads model_files/workshop_slim_00_l307000.txt (hex) and prints per-chunk details.
"""
...
def main():
    ...
    if not os.path.exists(hex_path):
        print('Hex input not found:', hex_path)
        return
    ...
    while pos < len(data) and shape_i < 1000:
        ...
        while inner < len(data):
            ...
            if inner_iterations > 10000:
                ...
                break
            if inner + 4 > len(data):
                print('incomplete chunk header at', inner)
                break
            ...
            inner += 4
            payload_start = inner
            total_len = length if length >= 4 else 4
            if length < 4:
                print('  WARNING: chunk length < 4 (malformed). Forcing minimum of 4 bytes')
            ...
            if inner <= header_start:
                print('  ERROR: parser did not advance; aborting to avoid loop')
                inner = len(data)
        pos = inner
        shape_i += 1
    print('\nDone')
