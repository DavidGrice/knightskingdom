...
// Overlay the real, LCA-derived OBJ/MTL geometry for EVERY brick that has
// one on disk. Previously this was gated behind the GLB footprint-ok report
// (only ~42 of 141 passed), so the other ~99 rendered as red parametric
// boxes despite their real OBJ existing -- the "picked a thin brick, got a
// red block" bug. The footprint guess only feeds the stud-grid stacking
// math (studs/heightPlates stay as-is); the OBJ is the true shape
// regardless. The semantic `shape` is retained so the cold-cache parametric
// fallback (BrickFactory, used only until the OBJ finishes loading) keeps
// the correct silhouette instead of flashing a generic box.
let objOverlayCount = 0;
...
// GLB stays generated as a non-live reference/fallback, still gated on the
// footprint-ok validation.
...
if (fs.existsSync(REPORT_PATH)) {
  ...
  for (const brickId of Object.keys(catalog)) {
    ...
    catalog[brickId].glbUrl = `/workshop/bricks/${brickId}.glb`;
    glbOverlayCount += 1;
  }
}
 ...
 * Entries: ${entries.length} (${objOverlayCount} with LCA-derived OBJ geometry, ${glbOverlayCount} GLB reference)
 */
`;
